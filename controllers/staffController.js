...
export async function changePasswordApi(req, res) {
  try {
    ...
    const isCurrentPasswordValid = await argon2.verify(staffMember.password, current_password);
    ...
    return createSuccessResponse(res, 200, true, "Technician password changed successfully");
  } catch (error) {
...
export const createJobServiceSheet = async (req, res) => {
  ...
  try {
    ...
    let jobServiceSheet =
      task.JobServiceSheet[0] || null;

    if (jobServiceSheet) {
      jobServiceSheet =
        await prisma.jobServiceSheet.update({
          where: {
      ...
      jobServiceSheet =
        await prisma.jobServiceSheet.create({
          data: jobSheetPayload
        });
    }
